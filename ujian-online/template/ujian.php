<?php
	if(!empty($_GET['m']) and $_GET['m']=='ujian'  and empty($_GET['op']) and !empty($_SESSION['sess_login'])){
	?>
	<div id="smallRight">
	...
	$col = "*";	
	$pk = "id_ujian";
	$sqlAll = "select $col from $tbl  where 1=1 ".$wh;
	$queryAll = mysql_query($sqlAll);
	$sql = "select id_ujian,mata_kuliah,jenis_ujian,tanggal, waktu, durasi from $tbl where 1=1 ".$wh." order by id_ujian desc limit $stpg,10";
