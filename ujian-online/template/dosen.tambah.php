<?php
	if(!empty($_GET['m']) and $_GET['m']=='dosen'  and !empty($_GET['op']) and $_GET['op']=='add'){		
	?>
	<div id="smallRight">
	...
	if(isset($_POST['simpan'])){
        ...
        ?>
        <script>
        	alert("Pengguna telah berhasil ditambhkan");
	    	...
	    	if (!$meta) {
	    		...
	    		if(strtoupper($meta->name)=='MARKER'){
					...
					case "password":
	    					...
	    					break;
					case "id_level":
	    					$inp="<input type='hidden' name='".$meta->name."' id='".$meta->name."' value='2'>";
							$name = "";
	    					break;
	  	    		case "keterangan":
