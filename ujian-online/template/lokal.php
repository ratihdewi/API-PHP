<?php
	if(!empty($_GET['m']) and $_GET['m']=='kelas'  and empty($_GET['op']) and !empty($_SESSION['sess_login'])){
	?>
	<div id="smallRight">
	<h2 align='center'>Data Lokal</h2>	
	<?php
	if(!empty($_GET['pg'])){
		$stpg = $_GET['pg'];
	}else{
	...
	$col = "*";	
	$pk = "id_kelas";
	$sqlAll = "select $col from $tbl order by id_kelas asc";
	$queryAll = mysql_query($sqlAll);
	$sql = "select $col from $tbl order by id_kelas asc limit $stpg,10";
