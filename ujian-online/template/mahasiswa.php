<?php
	if(!empty($_GET['m']) and $_GET['m']=='mahasiswa'  and empty($_GET['op']) and !empty($_SESSION['sess_login'])){
	?>
	<div id="smallRight">
	...
	$tbl = "mahasiswa";
	$col = "id_mahasiswa,username,nama,email";	
	$pk = "id_mahasiswa";
	$sqlAll = "select $col from $tbl order by id_mahasiswa desc";
	$queryAll = mysql_query($sqlAll);
	$sql = "select $col from $tbl order by id_mahasiswa desc limit $stpg,10";
