<?php
	if(!empty($_GET['m']) and $_GET['m']=='tipe'  and empty($_GET['op']) and !empty($_SESSION['sess_login'])){
	?>
	<div id="smallRight">
	<h2 align='center'>Data tipe_soal</h2>	
	<?php
	if(!empty($_GET['pg'])){
		$stpg = $_GET['pg'];
	}else{
	...
	$col = "*";	
	$pk = "id_tipe";
	$sqlAll = "select $col from $tbl order by id_tipe desc";
	$queryAll = mysql_query($sqlAll);
	$sql = "select $col from $tbl order by id_tipe desc limit $stpg,10";
