<?php
	if(!empty($_GET['m']) and $_GET['m']=='ujian'  and !empty($_GET['op']) and $_GET['op']=='hapus' and !empty($_GET['id']) ){		
	?>
	...
	if(isset($_POST['hapus'])){		      
		...
		mysql_query("delete from sla_matrix where id_soal in ( select id_soal from soal where $wh )");
        ?>
        <script>
...
<table id="box-table-a" summary="Meeting Results">
   <thead>
    ...
    <tbody>
    	...
    	<input type='submit' class='button' name='hapus' value='Hapus'>
    	</td>
    </body>
</table>
</form>
