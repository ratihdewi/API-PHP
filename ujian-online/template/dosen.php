<?php
	if(!empty($_GET['m']) and $_GET['m']=='dosen' and empty($_GET['op']) and !empty($_SESSION['sess_login'])){
	?>
	<div id="smallRight">
	...
	$tbl = "dosen";
	$col = "id_dosen,username,nama,email";	
	$pk = "id_dosen";
	$sqlAll = "select $col from $tbl order by id_dosen desc";
	$queryAll = mysql_query($sqlAll);
	$sql = "select $col from $tbl order by id_dosen desc limit $stpg,10";
