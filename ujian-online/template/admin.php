<?php
	if(!empty($_GET['m']) and $_GET['m']=='admin'  and empty($_GET['op']) and !empty($_SESSION['sess_login'])){
	?>
	<div id="smallRight">
	...
	$col = "";	
	$pk = "id_admin";
	$sqlAll = "select $col from $tbl order by id_admin desc";
	$queryAll = mysql_query($sqlAll);
	$sql = "select $col from $tbl order by id_admin desc limit $stpg,10";
